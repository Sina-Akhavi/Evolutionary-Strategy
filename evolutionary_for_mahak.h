...
SC_MODULE(mahak)
{
    ...
    void check_bag_limitation_and_repair_member(double *weights)
    {
        ...
        for (i = 0; i < OBJECT_NUMS; i++)
        {
            if (weights[i] > 0 && weights[i] < 0.5)
                ...
                weights[i] -= 0.5;
            // check if necessary to keep going 
        }
    ...
    void initialize()
    {
        ...
        int ceiling = 2;
        for (i = 0; i < OBJECT_NUMS; i++) {
            sigma[i] = (float) rand() / (RAND_MAX + 1.0);
            // printf("%f ", sigma[i]);
        }
    }
    ...
    void mutate()
    {
        ...
        for (i = 0; i < T_MEMBERS; i++)
        {
            for (j = 0; j < OBJECT_NUMS; j++)
            {
                T[i][j] = T[i][j] + sigma[j] * N01;
            }
        }
        // update sigmas
        for (i = 0; i < OBJECT_NUMS; i++)
        {
            sigma[i] = sigma[i] * exp(N01 - pdf(sigma[i] ,0, 1));
        }
    }
    ...
    void scale_to_0_1(double *arr, int size, int handle_existing)
    {
        ...
        // Handle existing values according to input
        double range = max_val - min_val;
        switch (handle_existing)
        {
        case 0: // Leave existing values unchanged
            break;
        case 1: // Clip existing values to boundaries
            for (int i = 0; i < size; ++i)
            {
                arr[i] = fmin(fmax(arr[i], 0.0), 1.0);
            }
            break;
        case 2: // Scale existing values proportionally (default)
            for (int i = 0; i < size; ++i)
            {
                arr[i] = (arr[i] - min_val + 0.1) / range;
            }
            break;
        default:
            printf("Invalid handle_existing value. Ignoring existing values.\n");
        }
    }
